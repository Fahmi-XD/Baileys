...
const makeSocket = (config) => {
    ...
    const validateConnection = async () => {
        ...
        helloMsg = WAProto_1.default.WAWa6.HandshakeMessage.fromObject(helloMsg);
        logger.info({ browser, helloMsg }, 'connected to WA');
        const init = WAProto_1.default.WAWa6.HandshakeMessage.encode(helloMsg).finish();
        const result = await awaitNextMessage(init);
        const handshake = WAProto_1.default.WAWa6.HandshakeMessage.decode(result);
        ...
        const payloadEnc = noise.encrypt(WAProto_1.default.WAWa6.ClientPayload.encode(node).finish());
        await sendRawMessage(WAProto_1.default.WAWa6.HandshakeMessage.encode({
            clientFinish: {
                static: keyEnc,
                payload: payloadEnc,
            },
