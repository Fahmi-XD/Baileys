...
const generateMac = (operation, data, keyId, key) => {
    const getKeyData = () => {
        let r;
        switch (operation) {
            case WAProto_1.default.WAServerSync.SyncdMutation.SyncdOperation.SET:
                r = 0x01;
                break;
            case WAProto_1.default.WAServerSync.SyncdMutation.SyncdOperation.REMOVE:
                r = 0x02;
                break;
        }
...
const makeLtHashGenerator = ({ indexValueMap, hash }) => {
    ...
    return {
        mix: ({ indexMac, valueMac, operation }) => {
            ...
            if (operation === WAProto_1.default.WAServerSync.SyncdMutation.SyncdOperation.REMOVE) {
                if (!prevOp) {
...
const encodeSyncdPatch = async ({ type, index, syncAction, apiVersion, operation }, myAppStateKeyId, state, getAppStateSyncKey) => {
    ...
    const dataProto = WAProto_1.default.WASyncAction.SyncActionData.fromObject({
        index: indexBuffer,
        value: syncAction,
        padding: new Uint8Array(0),
        version: apiVersion
    });
    const encoded = WAProto_1.default.WASyncAction.SyncActionData.encode(dataProto).finish();
...
const decodeSyncdMutations = async (msgMutations, initialState, getAppStateSyncKey, onMutation, validateMacs) => {
    ...
    for (const msgMutation of msgMutations) {
        ...
        const operation = 'operation' in msgMutation ? msgMutation.operation : WAProto_1.default.WAServerSync.SyncdMutation.SyncdOperation.SET;
        ...
        const syncAction = WAProto_1.default.WASyncAction.SyncActionData.decode(result);
        if (validateMacs) {
...
const extractSyncdPatches = async (result, options) => {
    ...
    await Promise.all(collectionNodes.map(async (collectionNode) => {
        ...
        if (snapshotNode && !!snapshotNode.content) {
            ...
            const blobRef = WAProto_1.default.WAServerSync.ExternalBlobReference.decode(snapshotNode.content);
            const data = await (0, exports.downloadExternalBlob)(blobRef, options);
            snapshot = WAProto_1.default.WAServerSync.SyncdSnapshot.decode(data);
        }
        for (let { content } of patches) {
            if (content) {
                ...
                const syncd = WAProto_1.default.WAServerSync.SyncdPatch.decode(content);
                if (!syncd.version) {
...
const downloadExternalPatch = async (blob, options) => {
    ...
    const syncData = WAProto_1.default.WAServerSync.SyncdMutations.decode(buffer);
    return syncData;
};
...
const chatModificationToAppPatch = (mod, jid) => {
    const OP = WAProto_1.default.WAServerSync.SyncdMutation.SyncdOperation;
