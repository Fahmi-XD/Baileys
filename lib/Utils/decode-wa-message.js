...
function decodeMessageNode(stanza, meId, meLid) {
    ...
    if ((0, WABinary_1.isJidUser)(from)) {
        ...
        logger_1.default.debug("Ini adalah Pesan Newsletter");
        msgType = "newsletter";
        chatId = from;
        author = participant;
    }
    ...
    if (key.fromMe) {
        fullMessage.status = WAProto_1.default.WAWeb.WebMessageInfo.Status.SERVER_ACK;
    }
    return {
        fullMessage,
        author,
            ...
            if (Array.isArray(stanza.content)) {
                for (const { tag, attrs, content } of stanza.content) {
                    ...
                    if (tag === 'verified_name' && content instanceof Uint8Array) {
                        const cert = WAProto_1.default.WAVnameCert.VerifiedNameCertificate.decode(content);
                        const details = WAProto_1.default.WAVnameCert.VerifiedNameCertificate.Details.decode(cert.details);
                    ...
                    try {
                        ...
                        let msg = WAProto_1.default.WAE2E.Message.decode(String(tag) === "plaintext" ? msgBuffer : (0, generics_1.unpadRandomMax16)(msgBuffer));
                        ...
                        logger.error({ key: fullMessage.key, err }, 'failed to decrypt message');
                        fullMessage.messageStubType = WAProto_1.default.WAWeb.WebMessageInfo.StubType.CIPHERTEXT;
