...
const makeNoiseHandler = ({ keyPair: { private: privateKey, public: publicKey }, NOISE_HEADER, mobile, logger, routingInfo }) => {
    ...
    return {
        ...
        processHandshake: ({ serverHello }, noiseKey) => {
            ...
            if (mobile) {
                WAProto_1.default.WACert.CertChain.NoiseCertificate.decode(certDecoded);
            }
            else {
                const { intermediate: certIntermediate } = WAProto_1.default.WACert.CertChain.decode(certDecoded);
                const { issuerSerial } = WAProto_1.default.WACert.CertChain.NoiseCertificate.Details.decode(certIntermediate.details);
